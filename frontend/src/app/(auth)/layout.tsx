...
export default function AuthLayout({
  ...
  return (
    // Start content below the floating top bar on every size (it's absolute, so
    // it reserves no space) and pad top/bottom so a tall card (signup) never
    // overlaps the bar or sticks to the screen edge.
    <div className="relative flex min-h-screen items-start justify-center px-4 pb-12 pt-24 md:pt-28">
      <Backdrop />
      ...
      {/* Mobile (<md): brand stacked above the card. Desktop (>=md): brand on one
          side and the card on the other (two columns) so the card gets its own
          breathing room. The brand stays centered (not edge-aligned). */}
      <div className="relative z-10 flex w-full max-w-sm animate-fade-in flex-col items-center gap-8 md:max-w-4xl md:flex-row md:items-center md:justify-center md:gap-16">
        <Link
          href="/"
